    ...
    for epoch in range(epochs):
        ...
        for batch in iterator:
            ...
            with torch.amp.autocast(device_type='cuda', dtype=torch.float16):
                inputs = batch["input_gray"].to(device, dtype=torch.float16)
            ...
            with torch.no_grad():
                ...
                # total ‖∇θ‖₂   (skip params that did not receive a grad)
        ...
        if local_rank == 0:
            ...
            if epoch % 10 == 0 or epoch == epochs - 1:
                torch.save(model.module.state_dict(), os.path.join(model_dir, f"gray1c_epoch_{epoch}.pth"))
        dist.barrier()
